...
<%
	...
	String title = "Committee Meeting: " + meeting.getCommitteeName()
			+ " - " + df.format(meeting.getMeetingDateTime());
	String calNo = null;

	Addendum addendum = meeting.getAddendums().get(
			meeting.getAddendums().size() - 1);
...
<%
	...
	chair = chair.replace("?", "");
%>
<div><b>Agenda:</b> <%=calNo%> / <b>Chair:</b> <a
	href="<%=appPath%>/search/?term=chair:<%=java.net.URLEncoder.encode("\""
					+ meeting.getCommitteeChair() + "\"",
					OpenLegConstants.ENCODING)%>"><%=chair%></a>
/ <b>Location:</b> <%=meeting.getLocation()%> <b>Addendum:</b> <%=addendum.getAddendumId()%>
/ <b>Published:</b> <%=addendum.getPublicationDateTime()%> / <b>Week
of:</b> <%=addendum.getWeekOf()%></div>
<div id="committeeNotes">
<%
	if (meeting.getNotes() != null
			&& meeting.getNotes().trim().length() > 0) {
%>
<h3>Notes</h3>
<%
...
<div class="billSummary"
	...
	onclick="location.href='/legislation/bill/<%=billNo%>'"><a
	href="/legislation/bill/<%=billNo%>"><%=billNo%>: <%=bill.getTitle()%></a>
<div style="font-size: 90%; color: #777777;"><%=bill.getSummary()%>
/ <%
...
<ul>
 ...
 %> / <%=vote.getAbstains().size()%>
	Abstains<%
 	}
 %> <%
 	if (vote.getExcused() != null) {
 %> / <%=vote.getExcused().size()%>
	Excused<%
 	}
 %>
	</li>
...
<%
			...
			System.err.println("couldn't render bill: "
