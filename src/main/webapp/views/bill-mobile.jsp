...
<b>Co-sponsor(s):</b>
 ...
 cp = it.next();
 %>
 <a href="<%=appPath%>/search/?format=mobile&term=sponsor:<%=java.net.URLEncoder.encode(cp.getFullname(),"utf-8")%>" class="sublink"><%=cp.getFullname()%></a><%if (it.hasNext()){%>, <%} %><%} %>
<br/>

 <%} %>

 <%if (bill.getCurrentCommittee()!=null){ %>
 <b>Committee:</b> <a href="<%=appPath%>/search/?format=mobile&term=committee:<%=java.net.URLEncoder.encode(bill.getCurrentCommittee(),"utf-8")%>" class="sublink"><%=bill.getCurrentCommittee()%></a>
<br/>
<%} %>

  <%if (bill.getLaw()!=null){ %>
 <b>Law:</b> <a href="<%=appPath%>/search/?format=mobile&term=<%=java.net.URLEncoder.encode("\"" + bill.getLaw()+"\"","utf-8")%>" class="sublink"><%=bill.getLaw()%></a> /
 <%} %>

  <%if (bill.getLawSection()!=null){ %>
 <b>Law Section:</b> <a href="<%=appPath%>/search/?format=mobile&term=<%=java.net.URLEncoder.encode("\"" + bill.getLawSection()+"\"","utf-8")%>" class="sublink"><%=bill.getLawSection()%></a><br/>
 <%} %>
