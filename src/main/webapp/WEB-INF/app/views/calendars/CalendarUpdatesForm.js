...
export default function CalendarUpdatesForm({ updateValues, aWeekAgo, todaysDate }) {

  React.useEffect(() => {
    let calendarUpdateFrom = document.getElementById("updateFrom")
    calendarUpdateFrom.value = aWeekAgo
    let calendarUpdateTo = document.getElementById("updateTo")
    calendarUpdateTo.value = todaysDate
  }, [])

  // Updates the term query param when the form is submitted.
  const onSubmitCalendarUpdatesSearch = (e) => {
    e.preventDefault()
  ...
  return (
    <div>
      ...
      <form onSubmit={onSubmitCalendarUpdatesSearch}>
        <div className="flex flex-wrap">
          <div className="flex-grow mr-8">

            <h1 className="p-5 font-bold">Show calendar updates during the following date range</h1>
