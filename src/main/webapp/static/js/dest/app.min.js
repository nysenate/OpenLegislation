...
var ASC=document.createElement("i");ASC.className="icon-arrow-long-up";var DESC=document.createElement("i");DESC.className="icon-arrow-long-down","ASC"==$scope.sort?event.toElement.appendChild(ASC):event.toElement.appendChild(DESC)}function formatSelectedMismatchType(mismatchType){return void 0==mismatchType||""==mismatchType.split("(")[0].trim()?void 0:mismatchType}function resetPagination(){$scope.pagination.reset(),$scope.pagination.setTotalItems(0),$scope.pagination.itemsPerPage=10}
...
function initDatasource(){$scope.datasource.selected=$scope.datasource.values[0],$routeParams.hasOwnProperty(searchParams.datasource)&&$scope.datasource.values.forEach(function(datasource){datasource.value===$routeParams[searchParams.datasource]&&($scope.datasource.selected=datasource)})}function initContentType(){$routeParams.hasOwnProperty(searchParams.contentType)&&($scope.selectedTab=contentTypes.indexOf($routeParams[searchParams.contentType]))}function initMismatchStatus(){$routeParams.hasOwnProperty(searchParams.mismatchStatus)&&($scope.selectedStatus=$routeParams[searchParams.mismatchStatus])}function initMismatchType(){$routeParams.hasOwnProperty(searchParams.mismatchType)&&($scope.selectedMismatchType=$routeParams[searchParams.mismatchType])}function initPage(){$routeParams.hasOwnProperty(searchParams.currentPage)&&($scope.currentPage=$routeParams[searchParams.currentPage],$scope.pagination.currPage=$scope.currentPage)}function initializeParameters(){initializeDate(),initDatasource(),initContentType(),initMismatchStatus(),initMismatchType(),initPage()}const dateFormat="YYYY-MM-DD",contentTypes=["BILL","CALENDAR","AGENDA"],searchParams={date:"date",datasource:"source",contentType:"content",mismatchStatus:"status",mismatchType:"type",currentPage:"page"};$scope.datasource={values:[{value:"LBDC",label:"LBDC - OpenLegislation"},{value:"NYSENATE",label:"OpenLegislation - NYSenate.gov"}],selected:{}},$scope.selectedStatus="NEW",// Show all open issues by default.
$scope.selectedTab=0,// Select Bills tab by default.
$scope.date={},$scope.loading=!1,// TODO remove this using promises?
$scope.pagination=angular.extend({},paginationModel),$scope.diffLoading=!1,$scope.orderby="OBSERVED_DATE",$scope.sort="DESC",$scope.currentPage=1,$scope.showGoto=!1,$scope.selectedMismatchType="All",$scope.mismatchResponse={mismatches:[],error:!1,errorMessage:""},$scope.mismatchStatusSummary={summary:{},error:!1,errorMessage:""},$scope.mismatchTypeSummary={summary:{},error:!1,errorMessage:""},$scope.mismatchContentTypeSummary={summary:{},error:!1,errorMessage:""},/* --- Input callbacks --- */
$scope.onDateChange=function(){$scope.date=moment($scope.pickedDate),$location.search(searchParams.date,$scope.date.format(dateFormat)).replace(),$route.reload()},$scope.onDatasourceChange=function(){resetPagination(),$scope.updateMismatchStatusSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches(),$location.search(searchParams.datasource,$scope.datasource.selected.value)},$scope.onTabChange=function(){resetPagination(),$scope.updateMismatches(),$scope.updateMismatchContentTypeSummary(),$location.search(searchParams.contentType,contentTypes[$scope.selectedTab])},
// update mismatch when user change status
$scope.onStatusChange=function(){resetPagination(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatches(),$location.search(searchParams.mismatchStatus,$scope.selectedStatus)},$scope.onMismatchTypeChange=function(){resetPagination(),$scope.updateMismatches(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary(),$location.search(searchParams.mismatchType,$scope.selectedMismatchType)},$scope.onPageChange=function(pageNum,contentType){contentType===selectedContentType()&&$scope.updateMismatches(),$location.search(searchParams.currentPage,pageNum)},$scope.onGotoChange=function(){$scope.pagination.currPage=$scope.currentPage},/* --- Content update functions --- */
$scope.updateMismatchStatusSummary=function(){$scope.mismatchStatusSummary.error=!1,mismatchSummaryApi.getMismatchStatusSummary($scope.datasource.selected.value,$scope.date._i).then(function(mismatchSummary){$scope.mismatchStatusSummary.summary=mismatchSummary.summary})["catch"](function(response){$scope.mismatchStatusSummary.error=!0,$scope.mismatchStatusSummary.errorMessage=response.statusText})},$scope.updateMismatchTypeSummary=function(){$scope.mismatchTypeSummary.error=!1,mismatchSummaryApi.getMismatchTypeSummary($scope.datasource.selected.value,$scope.date._i,$scope.selectedStatus).then(function(mismatchSummary){$scope.mismatchTypeSummary.summary=mismatchSummary,$scope.mismatchTypesSummaryShow=Object.keys($scope.mismatchTypeSummary.summary.typeCount.items)})["catch"](function(response){$scope.mismatchTypeSummary.error=!0,$scope.mismatchTypeSummary.errorMessage=response.statusText})},$scope.updateMismatchContentTypeSummary=function(){$scope.mismatchContentTypeSummary.error=!1,mismatchSummaryApi.getMismatchContentTypeSummary($scope.datasource.selected.value,$scope.date._i,$scope.selectedStatus,formatSelectedMismatchType($scope.selectedMismatchType)).then(function(mismatchSummary){$scope.mismatchContentTypeSummary.summary=mismatchSummary.summary})["catch"](function(response){$scope.mismatchContentTypeSummary.error=!0,$scope.mismatchContentTypeSummary.errorMessage=response.statusText})},$scope.updateOrder=function(column,$event){$scope.orderby==column?"DESC"==$scope.sort?$scope.sort="ASC":$scope.sort="DESC":($scope.orderby=column,$scope.sort="DESC"),$scope.updateMismatches(),updateOrderIcon($event)},$scope.updateMismatches=function(){$scope.loading=!0,$scope.mismatchResponse.error=!1,$scope.mismatchResponse.mismatches=[],spotcheckMismatchApi.getMismatches($scope.datasource.selected.value,selectedContentType(),$scope.selectedStatus,formatSelectedMismatchType($scope.selectedMismatchType),$scope.formatDate($scope.date),$scope.pagination.getLimit(),$scope.pagination.getOffset(),$scope.orderby,$scope.sort).then(function(result){$scope.pagination.setTotalItems(result.pagination.total),$scope.mismatchResponse.mismatches=result.mismatches,$scope.loading=!1,$scope.currentPage=$scope.pagination.currPage,$scope.showGoto=result.pagination.total>$scope.pagination.getLimit()})["catch"](function(response){$scope.loading=!1,$scope.mismatchResponse.error=!0,$scope.mismatchResponse.errorMessage=response.statusText})},$scope.numberWithCommas=function(x){return void 0==x||""==x?null:x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")},
...
mismatchId:mismatch.id,issueId:mismatch.issue};spotcheckMismatchTrackingAPI.save(params,function(response){})}$("#report-page-toast"+mismatch.id).fadeIn("5000"),$("#report-page-toast"+mismatch.id).fadeOut("slow")},$scope.formatDate=function(date){return date.format(dateFormat)},$scope.confirmIgnoreMismatch=function(mismatch){var confirm=$mdDialog.confirm().title("Ignore mismatch?").ok("Yes").cancel("No");$mdDialog.show(confirm).then(function(){ignoreMismatch(mismatch)})},
// show the diff window.
$scope.showDetailedDiff=function(mismatchRow){mismatchRow.diffLoading=!0,setTimeout(function(){$mdDialog.show({templateUrl:"mismatchDetailWindow",controller:"detailDialogCtrl",locals:{mismatchRow:mismatchRow,source:$scope.datasource.selected.value,contentType:selectedContentType()}}),mismatchRow.diffLoading=!1},10)};var inverseMismatchTypeMap=function(obj){var map=new Map;return angular.forEach(Object.keys(obj),function(k){map[obj[k]]=k}),map};$scope.mismatchTypeSortFunction=function(option){return $scope.mismatchTypeSummary.summary.typeCount.items[option]},/**
     * Updates the total mismatch count of each content type's tab
     * for the selected mismatch status.
     */
$scope.getSummaryCountForContentType=function(contentType){return null==$scope.mismatchContentTypeSummary.summary[contentType]?"":$scope.mismatchContentTypeSummary.summary[contentType][$scope.selectedStatus]||""},$scope.init=function(){resetPagination(),initializeParameters(),$scope.mismatchTypes=Object.values(window.mismatchMap),$scope.mismatchTypesShow=window.mismatchMap,$scope.inverseMismatchTypeMap=inverseMismatchTypeMap(window.mismatchMap),$scope.updateMismatchStatusSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches()},$scope.init()}function mismatchSummaryApi($resource){/**
