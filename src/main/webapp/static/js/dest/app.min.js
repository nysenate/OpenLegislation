...
$scope.onStatusChange=function(){resetPagination(),$scope.updateMismatchSummary(),$scope.updateMismatches()},$scope.formatDate=function(date){return date.format(dateFormat)},$scope.confirmIgnoreMismatch=function(mismatch){var confirm=$mdDialog.confirm().title("Ignore mismatch?").ok("Yes").cancel("No");$mdDialog.show(confirm).then(function(){ignoreMismatch(mismatch)})},$scope.showDetailedDiff=function(mismatchRow){$mdDialog.show({templateUrl:"mismatchDetailWindow",controller:"detailDialogCtrl",locals:{mismatchRow:mismatchRow,source:$scope.datasource.selected.value,contentType:selectedContentType()}})},/**
