...
function ignoreMismatch(mismatch){var params={mismatchId:mismatch.id,ignoreLevel:"IGNORE_UNTIL_RESOLVED"};mismatchIgnoreApi.save(params,function(response){$scope.updateMismatchTypeSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchStatusSummary(),$scope.updateMismatches()})}function initializeDate(){$routeParams.hasOwnProperty("date")?($scope.date=moment($routeParams.date,dateFormat),$scope.pickedDate=new Date($scope.date)):($scope.date=moment().startOf("day"),$location.search("date",$scope.date.format(dateFormat)).replace(),$scope.pickedDate=new Date(moment().startOf("day"))),$scope.maxDate=new Date(moment().startOf("day")),$mdDateLocale.formatDate=function(date){return moment(date).format(dateFormat)}}function selectedContentType(){return contentTypes[$scope.selectedTab]}const dateFormat="YYYY-MM-DD",contentTypes=["BILL","CALENDAR","AGENDA"];$scope.datasource={values:[{value:"LBDC",label:"LBDC - OpenLegislation"},{value:"NYSENATE",label:"OpenLegislation - NYSenate.gov"},{value:"OPENLEGDEV",label:"SOBI - XML"}],selected:{}},$scope.selectedStatue="NEW",// Show all open issues by default.
$scope.selectedTab=0,// Select Bills tab by default.
$scope.date={},$scope.loading=!1,// TODO remove this using promises?
$scope.pagination=angular.extend({},paginationModel),$scope.diffLoading=!1,$scope.orderby="OBSERVED_DATE",$scope.sort="DESC",$scope.currentPage=1,$scope.showGoto=!1,$scope.selectedMismatchType="All",$scope.mismatchResponse={mismatches:[],error:!1,errorMessage:""},$scope.mismatchStatusSummary={summary:{},error:!1,errorMessage:""},$scope.mismatchTypeSummary={summary:{},error:!1,errorMessage:""},$scope.mismatchContentTypeSummary={summary:{},error:!1,errorMessage:""},$scope.onDatasourceChange=function(){resetPagination(),$scope.updateMismatchStatusSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches()},$scope.onTabChange=function(){resetPagination(),$scope.updateMismatches(),$scope.updateMismatchContentTypeSummary()},$scope.onPageChange=function(pageNum,contentType){contentType==selectedContentType()&&$scope.updateMismatches()},$scope.updateMismatchStatusSummary=function(){$scope.mismatchStatusSummary.error=!1,mismatchSummaryApi.getMismatchStatusSummary($scope.datasource.selected.value,$scope.date._i).then(function(mismatchSummary){$scope.mismatchStatusSummary.summary=mismatchSummary.summary})["catch"](function(response){$scope.mismatchStatusSummary.error=!0,$scope.mismatchStatusSummary.errorMessage=response.statusText})},$scope.updateMismatchTypeSummary=function(){$scope.mismatchTypeSummary.error=!1,mismatchSummaryApi.getMismatchTypeSummary($scope.datasource.selected.value,$scope.date._i,$scope.selectedStatue).then(function(mismatchSummary){$scope.mismatchTypeSummary.summary=mismatchSummary,$scope.mismatchTypesSummaryShow=Object.keys($scope.mismatchTypeSummary.summary.typeCount.items)})["catch"](function(response){$scope.mismatchTypeSummary.error=!0,$scope.mismatchTypeSummary.errorMessage=response.statusText})},$scope.updateMismatchContentTypeSummary=function(){$scope.mismatchContentTypeSummary.error=!1,mismatchSummaryApi.getMismatchContentTypeSummary($scope.datasource.selected.value,$scope.date._i,$scope.selectedStatue,formatSelectedMismatchType($scope.selectedMismatchType)).then(function(mismatchSummary){$scope.mismatchContentTypeSummary.summary=mismatchSummary.summary})["catch"](function(response){$scope.mismatchContentTypeSummary.error=!0,$scope.mismatchContentTypeSummary.errorMessage=response.statusText})},$scope.onMismatchTypeChange=function(){resetPagination(),$scope.updateMismatches(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary()},$scope.updateOrder=function(column,$event){$scope.orderby==column?"DESC"==$scope.sort?$scope.sort="ASC":$scope.sort="DESC":($scope.orderby=column,$scope.sort="DESC"),$scope.updateMismatches(),updateOrderIcon($event)},$scope.updateMismatches=function(){$scope.loading=!0,$scope.mismatchResponse.error=!1,$scope.mismatchResponse.mismatches=[],spotcheckMismatchApi.getMismatches($scope.datasource.selected.value,selectedContentType(),$scope.selectedStatue,formatSelectedMismatchType($scope.selectedMismatchType),$scope.formatDate($scope.date),$scope.pagination.getLimit(),$scope.pagination.getOffset(),$scope.orderby,$scope.sort).then(function(result){$scope.pagination.setTotalItems(result.pagination.total),$scope.mismatchResponse.mismatches=result.mismatches,$scope.loading=!1,$scope.currentPage=$scope.pagination.currPage,$scope.showGoto=result.pagination.total>$scope.pagination.getLimit()})["catch"](function(response){$scope.loading=!1,$scope.mismatchResponse.error=!0,$scope.mismatchResponse.errorMessage=response.statusText})},$scope.onGotoChange=function(){$scope.pagination.currPage=$scope.currentPage},$scope.numberWithCommas=function(x){return void 0==x||""==x?null:x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")},
