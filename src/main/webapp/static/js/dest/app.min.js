...
<<<<<<< HEAD
function ignoreMismatch(mismatch){var params={mismatchId:mismatch.id,ignoreLevel:"IGNORE_UNTIL_RESOLVED"};mismatchIgnoreApi.save(params,function(response){$scope.updateMismatchSummary(),$scope.updateMismatches()})}function initializeDate(){$routeParams.hasOwnProperty("date")?($scope.date=moment($routeParams.date,dateFormat),$scope.pickedDate=new Date($scope.date)):($scope.date=moment().startOf("day"),$location.search("date",$scope.date.format(dateFormat)).replace(),$scope.pickedDate=new Date(moment().startOf("day"))),$scope.maxDate=new Date(moment().startOf("day")),$mdDateLocale.formatDate=function(date){return moment(date).format(dateFormat)}}function selectedContentType(){return contentTypes[$scope.selectedTab]}const dateFormat="YYYY-MM-DD",isoFormat="YYYY-MM-DDTHH:mm:ss",contentTypes=["BILL","CALENDAR","AGENDA"];$scope.datasource={values:[{value:"LBDC",label:"LBDC - OpenLegislation"},{value:"NYSENATE",label:"OpenLegislation - NYSenate.gov"}],selected:{}},$scope.status="OPEN",// Show all open issues by default.
$scope.selectedTab=0,// Select Bills tab by default.
$scope.date={},$scope.loading=!1,// TODO remove this using promises?
$scope.pagination=angular.extend({},paginationModel),$scope.diffLoading=!1,$scope.orderby="OBSERVED_DATE",$scope.sort="DESC",$scope.currentPage=1,$scope.showGoto=!1,$scope.mismatchResponse={mismatches:[],error:!1,errorMessage:""},$scope.summaryResponse={summary:{},error:!1,errorMessage:""},$scope.onDatasourceChange=function(){resetPagination(),$scope.updateMismatchSummary(),$scope.updateMismatches()},$scope.onTabChange=function(){resetPagination(),$scope.updateMismatches()},$scope.onPageChange=function(pageNum,contentType){contentType==selectedContentType()&&$scope.updateMismatches()},$scope.updateMismatchSummary=function(){$scope.summaryResponse.error=!1,mismatchSummaryApi.get($scope.datasource.selected.value,$scope.date.endOf("day").format(isoFormat)).then(function(mismatchSummary){$scope.summaryResponse.summary=mismatchSummary})["catch"](function(response){$scope.summaryResponse.error=!0,$scope.summaryResponse.errorMessage=response.statusText})},$scope.updateOrder=function(column,$event){$scope.orderby==column?"DESC"==$scope.sort?$scope.sort="ASC":$scope.sort="DESC":($scope.orderby=column,$scope.sort="DESC"),$scope.updateMismatches(),updateOrderIcon($event)},$scope.updateMismatches=function(){/**
         * Returns array of mismatch statuses corresponding to the selected status.
         */
function toMismatchStatus(status){return"OPEN"===status?["NEW","EXISTING","REGRESSION"]:"NEW"==status?["NEW"]:["RESOLVED"]}$scope.loading=!0,$scope.mismatchResponse.error=!1,$scope.mismatchResponse.mismatches=[],spotcheckMismatchApi.getMismatches($scope.datasource.selected.value,selectedContentType(),toMismatchStatus($scope.status),$scope.date.startOf("day").format(isoFormat),$scope.date.endOf("day").format(isoFormat),$scope.pagination.getLimit(),$scope.pagination.getOffset(),$scope.orderby,$scope.sort).then(function(result){$scope.pagination.setTotalItems(result.pagination.total),$scope.mismatchResponse.mismatches=result.mismatches,$scope.loading=!1,$scope.currentPage=$scope.pagination.currPage,$scope.showGoto=result.pagination.total>$scope.pagination.getLimit()})["catch"](function(response){$scope.loading=!1,$scope.mismatchResponse.error=!0,$scope.mismatchResponse.errorMessage=response.statusText})},$scope.onGotoChange=function(){$scope.pagination.currPage=$scope.currentPage},$scope.numberWithCommas=function(x){return void 0==x||""==x?null:x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")},
=======
function ignoreMismatch(mismatch){var params={mismatchId:mismatch.id,ignoreLevel:"IGNORE_UNTIL_RESOLVED"};mismatchIgnoreApi.save(params,function(response){$scope.updateMismatchTypeSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchStatusSummary(),$scope.updateMismatches()})}function initializeDate(){$routeParams.hasOwnProperty("date")?$scope.date=moment($routeParams.date,dateFormat):($scope.date=moment().startOf("day"),onDateChange())}function onDateChange(){$location.search("date",$scope.date.format(dateFormat)).replace()}function selectedContentType(){return contentTypes[$scope.selectedTab]}const dateFormat="YYYY-MM-DD",isoFormat="YYYY-MM-DDTHH:mm:ss",contentTypes=["BILL","CALENDAR","AGENDA"];$scope.datasource={values:[{value:"LBDC",label:"LBDC - OpenLegislation"},{value:"NYSENATE",label:"OpenLegislation - NYSenate.gov"}],selected:{}},$scope.status="OPEN",// Show all open issues by default.
$scope.selectedTab=0,// Select Bills tab by default.
$scope.date={},$scope.loading=!1,// TODO remove this using promises?
$scope.pagination=angular.extend({},paginationModel),$scope.diffLoading=!1,$scope.orderby="OBSERVED_DATE",$scope.sort="DESC",$scope.selectedMismatchType="ALL",$scope.mismatchResponse={mismatches:[],error:!1,errorMessage:""},$scope.summaryResponse={summary:{},error:!1,errorMessage:""},$scope.onDatasourceChange=function(){resetPagination(),$scope.updateMismatchStatusSummary(),$scope.updateMismatches()},$scope.onTabChange=function(){resetPagination(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatches()},$scope.onPageChange=function(pageNum,contentType){contentType==selectedContentType()&&$scope.updateMismatches()},$scope.updateMismatchStatusSummary=function(){$scope.summaryResponse.error=!1,mismatchSummaryApi.getMismatchStatusSummary($scope.datasource.selected.value,$scope.date.endOf("day").format(isoFormat)).then(function(mismatchSummary){$scope.summaryResponse.summary=mismatchSummary})["catch"](function(response){$scope.summaryResponse.error=!0,$scope.summaryResponse.errorMessage=response.statusText})},$scope.updateMismatchTypeSummary=function(){$scope.summaryResponse.error=!1,mismatchSummaryApi.getMismatchTypeSummary($scope.datasource.selected.value,$scope.date.endOf("day").format(isoFormat),$scope.status).then(function(mismatchSummary){$scope.summaryResponse.summary=mismatchSummary})["catch"](function(response){$scope.summaryResponse.error=!0,$scope.summaryResponse.errorMessage=response.statusText})},$scope.updateMismatchContentTypeSummary=function(){$scope.summaryResponse.error=!1,mismatchSummaryApi.getMismatchContentTypeSummary($scope.datasource.selected.value,$scope.date.endOf("day").format(isoFormat),$scope.status,$scope.selectedMismatchType).then(function(mismatchSummary){$scope.summaryResponse.summary=mismatchSummary})["catch"](function(response){$scope.summaryResponse.error=!0,$scope.summaryResponse.errorMessage=response.statusText})},$scope.onMismatchTypeChange=function(){resetPagination(),$scope.updateMismatches(),$scope.updateMismatchTypeSummary()},$scope.updateOrder=function(column,$event){$scope.orderby==column?"DESC"==$scope.sort?$scope.sort="ASC":$scope.sort="DESC":($scope.orderby=column,$scope.sort="DESC"),$scope.updateMismatches(),updateOrderIcon($event)},$scope.updateMismatches=function(){/**
         * Returns array of mismatch statuses corresponding to the selected status.
         */
function toMismatchStatus(status){return"OPEN"===status?["NEW","EXISTING","REGRESSION"]:"NEW"==status?["NEW"]:["RESOLVED"]}$scope.loading=!0,$scope.mismatchResponse.error=!1,$scope.mismatchResponse.mismatches=[],spotcheckMismatchApi.getMismatches($scope.datasource.selected.value,selectedContentType(),toMismatchStatus($scope.status),$scope.date.startOf("day").format(isoFormat),$scope.date.endOf("day").format(isoFormat),$scope.pagination.getLimit(),$scope.pagination.getOffset(),$scope.orderby,$scope.sort,$scope.selectedMismatchType).then(function(result){$scope.pagination.setTotalItems(result.pagination.total),$scope.mismatchResponse.mismatches=result.mismatches,$scope.loading=!1})["catch"](function(response){$scope.loading=!1,$scope.mismatchResponse.error=!0,$scope.mismatchResponse.errorMessage=response.statusText})},$scope.numberWithCommas=function(x){return void 0==x||""==x?null:x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")},
>>>>>>> New spotcheck mismatch summary Api
// update mismatch's issues, issue splits by comma
$scope.updateIssue=function(mismatch){if(""==mismatch.issue){// if issue is empty, then clear all related issues
var params={mismatchId:mismatch.id};spotcheckMismatchDeleteAllAPI["delete"](params,function(response){})}else{var params={// update issue
mismatchId:mismatch.id,issueId:mismatch.issue};spotcheckMismatchTrackingAPI.save(params,function(response){})}$("#report-page-toast"+mismatch.id).fadeIn("5000"),$("#report-page-toast"+mismatch.id).fadeOut("slow")},
// update mismatch when user change status
$scope.onStatusChange=function(){resetPagination(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches()},$scope.formatDate=function(date){return date.format(dateFormat)},$scope.confirmIgnoreMismatch=function(mismatch){var confirm=$mdDialog.confirm().title("Ignore mismatch?").ok("Yes").cancel("No");$mdDialog.show(confirm).then(function(){ignoreMismatch(mismatch)})},
// show the diff window.
$scope.showDetailedDiff=function(mismatchRow){mismatchRow.diffLoading=!0,setTimeout(function(){$mdDialog.show({templateUrl:"mismatchDetailWindow",controller:"detailDialogCtrl",locals:{mismatchRow:mismatchRow,source:$scope.datasource.selected.value,contentType:selectedContentType()}}),mismatchRow.diffLoading=!1},10)},$scope.onDateChange=function(){$scope.date=moment($scope.pickedDate),$location.search("date",$scope.date.format(dateFormat)).replace(),$route.reload()},/**
     * Updates the total mismatch count of each content type's tab
     * for the selected mismatch status.
     */
$scope.getSummaryCountForContentType=function(contentType){return null==$scope.summaryResponse.summary[contentType]?"":$scope.summaryResponse.summary[contentType][$scope.status]||""},$scope.init=function(){resetPagination(),initializeDate(),$scope.datasource.selected=$scope.datasource.values[0],$scope.updateMismatchStatusSummary(),$scope.updateMismatchContentTypeSummary(),$scope.updateMismatchTypeSummary(),$scope.updateMismatches(),$scope.mismatchTypes=Object.values(window.mismatchMap)},$scope.init()}function mismatchSummaryApi($resource){/**
     ...
     */
function getMismatchStatusSummary(datasource,date){return mismatchStatusSummaryApi.get({datasource:datasource,summaryDateTime:date}).$promise.then(createStatusSummary)}function getMismatchTypeSummary(datasource,date,status){return mismatchTypeSummaryApi.get({datasource:datasource,summaryDateTime:date,status:status}).$promise.then(createMismatchTypeSummary)}function getMismatchContentTypeSummary(datasource,date,status,type){return mismatchContentTypeSummaryApi.get({datasource:datasource,summaryDateTime:date,status:status,type:type}).$promise.then(createContentTypeSummary)}function createStatusSummary(response){return response.result}function createMismatchTypeSummary(response){return response.result}function createContentTypeSummary(response){return response.result}var mismatchStatusSummaryApi=$resource(adminApiPath+"/spotcheck/mismatches/summary/status"),mismatchTypeSummaryApi=$resource(adminApiPath+"/spotcheck/mismatches/summary/type"),mismatchContentTypeSummaryApi=$resource(adminApiPath+"/spotcheck/mismatches/summary/contenttype");return{getMismatchStatusSummary:getMismatchStatusSummary,getMismatchTypeSummary:getMismatchTypeSummary,getMismatchContentTypeSummary:getMismatchContentTypeSummary}}function spotcheckMismatchApi($resource){function getMismatches(datasource,contentType,mismatchStatuses,fromDate,toDate,limit,offset,orderBy,sort,error){var params={datasource:datasource,contentType:contentType,mismatchStatuses:mismatchStatuses,limit:limit,offset:offset,toDate:toDate,orderBy:orderBy,sort:sort,error:error};
// for resolve
return mismatchStatuses.indexOf("RESOLVED")!=-1&&(params.fromDate=fromDate),mismatchApi.get(params).$promise.then(parseMismatches)}function parseMismatches(response){var result={pagination:{total:0},mismatches:[]};return result.pagination.total=response.total,angular.forEach(response.result.items,function(mismatch){"NOT_IGNORED"===mismatch.ignoreStatus&&result.mismatches.push(createMismatch(mismatch))}),result}function createMismatch(mismatch){return{id:parseMismatchId(mismatch),status:parseStatus(mismatch),disableDiff:parseDisableDiff(mismatch),mismatchType:parseMismatchType(mismatch),observedDate:parseObservedDate(mismatch),referenceDate:parseReferenceDate(mismatch),reportDate:parseReportDate(mismatch),issue:parseIssues(mismatch),refType:parseRefType(mismatch),bill:parseBill(mismatch),source:parseRefType(mismatch),calNo:parseCalNo(mismatch),calType:parseCalType(mismatch),session:parseSession(mismatch),basePrintNo:parseBasePrintNo(mismatch),referenceData:parseReferenceData(mismatch),observedData:parseObserveredData(mismatch),diffLoading:!1,agendaNo:parseAgendaNo(mismatch),committee:parseCommittee(mismatch)}}function parseDisableDiff(mismatch){return"REFERENCE_DATA_MISSING"==mismatch.mismatchType||"OBSERVE_DATA_MISSING"==mismatch.mismatchType}function parseCalType(mismatch){if(void 0!=mismatch.key.type){var result="";return"ACTIVE_LIST"==mismatch.key.type&&(result="Active"),"FLOOR_CALENDAR"==mismatch.key.type&&(result="Floor"),"SUPPLEMENTAL_CALENDAR"==mismatch.key.type&&(result="Suppl"),result}}function parseSession(mismatch){return mismatch.key.session}function parseBasePrintNo(mismatch){return mismatch.key.basePrintNo}function parseMismatchId(mismatch){return mismatch.mismatchId}function parseReferenceData(mismatch){return mismatch.referenceData}function parseStatus(mismatch){return patternWords(mismatch.status)}function parseObserveredData(mismatch){return mismatch.observedData}function parseMismatchType(mismatch){return"OBSERVE_DATA_MISSING"==mismatch.mismatchType?("LBDC"==mismatch.dataSource?("BILL"==mismatch.contentType&&(mismatch.mismatchType="Missing LBDC Bill"),"CALENDAR"==mismatch.contentType&&(mismatch.mismatchType="Missing LBDC Cal"),"AGENDA"==mismatch.contentType&&(mismatch.mismatchType="Missing LBDC Agenda")):("BILL"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Bill"),"CALENDAR"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Cal"),"AGENDA"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Agenda")),mismatch.mismatchType):"REFERENCE_DATA_MISSING"==mismatch.mismatchType?("LBDC"==mismatch.dataSource?("BILL"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Bill"),"CALENDAR"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Cal"),"AGENDA"==mismatch.contentType&&(mismatch.mismatchType="Missing OpenLeg  Agenda")):("BILL"==mismatch.contentType&&(mismatch.mismatchType="Missing NYSenate.gov  Bill"),"CALENDAR"==mismatch.contentType&&(mismatch.mismatchType="Missing NYSenate.gov  Cal"),"AGENDA"==mismatch.contentType&&(mismatch.mismatchType="Missing NYSenate.gov  Agenda")),mismatch.mismatchType):mismatchMap[mismatch.mismatchType]}function parseReferenceDate(mismatch){return moment(mismatch.referenceDateTime).format(DATE_FORMAT)}function parseObservedDate(mismatch){return moment(mismatch.observedDateTime).format(DATE_FORMAT)}function parseReportDate(mismatch){return moment(mismatch.reportDateTime).format(DATE_FORMAT)}function parseIssues(mismatch){return mismatch.issueIds.items.join(", ")}function parseRefType(mismatch){return referenceTypeDisplayMap[mismatch.referenceType]}function parseBill(mismatch){return mismatch.key.printNo||""}function parseCalNo(mismatch){return mismatch.key.calNo||""}function parseAgendaNo(mismatch){return void 0==mismatch.key.agendaId||null==mismatch.key.agendaId?"":mismatch.key.agendaId.number==-1?"N/A":"DEFAULT"==mismatch.key.addendum?mismatch.key.agendaId.number:mismatch.key.agendaId.number+patternWords(mismatch.key.addendum)}function parseCommittee(mismatch){return null==mismatch.key.committeeId?"":mismatch.key.committeeId.name}function patternWords(input){var lower=input.toLowerCase();return lower.charAt(0).toUpperCase()+lower.slice(1)}const DATE_FORMAT="M-D-YY h:mm a";var mismatchApi=$resource(adminApiPath+"/spotcheck/mismatches");return{getMismatches:getMismatches}}var openApp=angular.module("open",
// External modules
...
scope.menu={sections:[]};var $sections=$($elem.context).children("menu-section");angular.forEach($sections,function(_s){var section={title:_s.title,items:[]};angular.forEach($(_s).children("menu-item"),function(_i){var item={url:$(_i).attr("url"),title:$(_i).text(),icon:$(_i).attr("icon"),target:$(_i).attr("target")};section.items.push(item)}),scope.menu.sections.push(section)}),
...
spotcheckModule.controller("SpotcheckCtrl",["$scope","$routeParams","$location","$timeout","$filter","$mdDialog",function($scope,$routeParams,$location,$timeout,$filter,$mdDialog){$scope.rtmap={},$scope.rtDispMap={},$scope.mtmap={},$scope.init=function(rtmap,rtDispMap,mtmap){$scope.rtmap=reportTypeMap=rtmap,$scope.rtDispMap=reportTypeDispMap=rtDispMap,$scope.mtmap=mismatchTypeMap=mtmap,$scope.setHeaderVisible(!0),$scope.setHeaderText("View Spotcheck Reports")},$scope.getReportURL=function(reportType,reportRunTime){return ctxPath+"/admin/report/spotcheck/"+$filter("reportTypeRefName")(reportType)+"/"+$filter("moment")(reportRunTime,"YYYY-MM-DD[T]HH:mm:ss.SSS")},/**
     ...
     */
$scope.getContentId=function(reportType,key){return $filter("contentId")(key,reportType)},/**
     ...
     * @param key - A content key that references a piece of legislative content
     */
$scope.getContentUrl=function(reportType,key){return $filter("contentUrl")(key,reportType)},
// Searches through the prior mismatches of a mismatch to find the date that it was first opened
$scope.findFirstOpenedDates=function(mismatch,observation){if("NEW"==mismatch.status)return{reportDateTime:observation.reportDateTime,referenceDateTime:observation.refDateTime};for(var index in mismatch.prior.items)if("NEW"==mismatch.prior.items[index].status)return{reportDateTime:mismatch.prior.items[index].reportId.reportDateTime,referenceDateTime:mismatch.prior.items[index].reportId.referenceDateTime};return{reportDateTime:"Unknown",referenceDateTime:"Unknown"}},
// Generates a mismatch id
$scope.getMismatchId=function(observation,mismatch){return JSON.stringify(observation.key)+"-"+mismatch.mismatchType},$scope.extractMismatchRows=function(observations,refType){var mismatchRows=[];return angular.forEach(observations,function(obs){angular.forEach(obs.mismatches.items,function(m){var chips=[];m.ignoreStatus&&"NOT_IGNORED"!==m.ignoreStatus&&chips.push(m.ignoreStatus),angular.forEach(m.issueIds.items,function(issueId){chips.push(issueId)}),mismatchRows.push({key:obs.key,keyString:$scope.getContentId(refType,obs.key),observation:obs,mismatch:m,type:m.mismatchType,status:m.status,observed:obs.observedDateTime,reportDateTime:obs.reportDateTime,firstOpened:$scope.findFirstOpenedDates(m,obs).reportDateTime,refData:m.referenceData,obsData:m.observedData,mismatchId:$scope.getMismatchId(obs,m),refType:refType,chips:chips})})}),mismatchRows},$scope.showSummaryDetails=function(summary){$scope.notImplementedDialog()}}]),spotcheckModule.directive("mismatchDiff",["$timeout",function($timeout){return{restrict:"E",scope:{left:"=",right:"="},template:"<span ng-class='{preformatted: pre, \"word-wrap\": !pre}' style='line-height: {{lineHeight}}px'><line-numbers ng-if='pre && showLines' line-end='lines'></line-numbers><semantic-diff left-obj='left' right-obj='right'></semantic-diff></span></span>",link:function($scope,$element,$attrs){$scope.pre=!$attrs.hasOwnProperty("noPre"),$scope.wrap=!$attrs.hasOwnProperty("noWrap"),$scope.showLines="false"!==$attrs.showLines,$scope.lines=1,$scope.lineHeight=20,$scope.adjustLineCount=function(){$scope.pre||$attrs.hasOwnProperty("noPre")||($scope.pre=$scope.left.split(/\n/).length>1||$scope.right.split(/\n/).length>1),$scope.pre&&$scope.showLines&&$timeout(function(){var childElement=$element.children()[0],elementHeight=childElement.offsetHeight;$scope.lines=Math.ceil(elementHeight/$scope.lineHeight),$scope.pre=$scope.lines>1},100)},$scope.$watchGroup(["left","right"],$scope.adjustLineCount)}}}]),spotcheckModule.directive("diffSummary",function(){return{restrict:"E",scope:{fullDiff:"="},template:"<div ng-repeat='diff in selectedDiffs'><span class='diff-summary-header'>Lines {{diff.startLineNum}} to {{diff.endLineNum}}:<br></span><div class='preformatted'><line-numbers line-start='diff.startLineNum' line-end='diff.endLineNum'></line-numbers><span ng-bind='diff.startText'></span><span ng-repeat='seg in diff.segments'><span ng-if='seg.operation === \"EQUAL\"' ng-bind='seg.text'></span><ins ng-if='seg.operation === \"INSERT\"' ng-bind='seg.text'></ins><del ng-if='seg.operation === \"DELETE\"' ng-bind='seg.text'></del></span><span ng-bind='diff.endText'></span></div><br ng-if='!$last'><br ng-if='!$last'><md-divider ng-if='!$last'></md-divider></div>",link:function($scope,$element,$attrs){$scope.selectedDiffs=[],$scope.$watch("selectedDiffs",function(){console.log($scope.selectedDiffs)},!0);var currentLineNum=1,currentLineText="",currentDiff=null;
// Isolate differences from full text
for(var iSeg in $scope.fullDiff){var segment=$scope.fullDiff[iSeg],segLines=segment.text.split(/\n/);["DELETE","INSERT"].indexOf(segment.operation)>=0?(null==currentDiff&&(currentDiff={startText:currentLineText,startLineNum:currentLineNum,segments:[]}),currentDiff.segments.push(segment)):(null!=currentDiff&&(segLines.length>2?(currentDiff.endText=segLines[0],currentDiff.endLineNum=currentLineNum,$scope.selectedDiffs.push(currentDiff),currentDiff=null):currentDiff.segments.push(segment)),currentLineText=segLines.length>1?segLines[segLines.length-1]:currentLineText+segLines[0]),currentLineNum+=segLines.length-1}null!=currentDiff&&(currentDiff.endText="",currentDiff.endLineNum=currentLineNum,$scope.selectedDiffs.push(currentDiff)),$scope.singleLine=function(diff){return diff.startLineNum==diff.endLineNum}}}}),angular.module("open.spotcheck").controller("detailDialogCtrl",["$scope","$mdDialog","mismatchRow","source","contentType",detailDialogCtrl]),/** --- Report Summary Controller --- */
angular.module("open.spotcheck").controller("SpotcheckSummaryCtrl",["$scope","$filter","$routeParams","$location","SpotcheckSummaryAPI",function($scope,$filter,$routeParams,$location,SpotcheckSummaryAPI){$scope.reportSummaries=[],$scope.filteredReportSummaries=[],$scope.dataProvider=[],$scope.response=null,$scope.loadingSummaries=!1,$scope.summariesNotFound=!1,$scope.showErrorlessReports=!1,$scope.resultsPerPage=20,$scope.params={summaryType:"LBDC_DAYBREAK",inputStartDate:null,inputEndDate:null},$scope.pagination={currPage:1,itemsPerPage:10},$scope.init=function(){"type"in $routeParams&&($scope.params.summaryType=$filter("reportType")($routeParams.type)||"all"),$routeParams.hasOwnProperty("endDate")&&moment($routeParams.endDate).isValid()?$scope.endDate=moment($routeParams.endDate):$scope.endDate=moment(),$routeParams.hasOwnProperty("startDate")&&moment($routeParams.startDate).isValid()?$scope.startDate=moment($routeParams.startDate):$scope.startDate=moment($scope.endDate).subtract(3,"months"),$scope.params.inputStartDate=moment($scope.startDate).toDate(),$scope.params.inputEndDate=moment($scope.endDate).toDate()},$scope.getSummaries=function(){console.log("getting new summaries");var summaryType="all"!==$scope.params.summaryType?$filter("reportTypeRefName")($scope.params.summaryType):[];$scope.loadingSummaries=!0,$scope.response=SpotcheckSummaryAPI.get({startDate:$scope.startDate.format(),endDate:$scope.endDate.endOf("day").format(),reportType:summaryType},function(){$scope.response.success&&($scope.reportSummaries=$scope.response.reports.items,console.log("summaries received"),$scope.setSummarySearchParams(),$scope.loadingSummaries=!1,$scope.summariesNotFound=!1,$scope.filterSummaries())},function(response){console.log(response),$scope.loadingSummaries=!1,$scope.summariesNotFound=!0})},
// Watch the params for changes and make a request for report summaries if there were changes
$scope.$watchCollection("params",function(){$scope.endDate=moment($scope.params.inputEndDate),$scope.startDate=moment($scope.params.inputStartDate),$scope.getSummaries()}),$scope.setSummarySearchParams=function(){$scope.clearSearchParams(),$scope.setSearchParam("type",$filter("reportTypeRefName")($scope.params.summaryType),"all"!==$scope.params.summaryType)},
// Compute the total number of mismatches for a given type.
$scope.computeMismatchCount=function(summaryItem,type){var defaultFilter=$filter("default"),mismatchType=summaryItem.mismatchTypes[type];return mismatchType?defaultFilter(mismatchType.NEW,0)+defaultFilter(mismatchType.EXISTING,0)+defaultFilter(mismatchType.REGRESSION,0):0},
// Compute the difference between open issues and resolved issues. Set 'abs' to true to
// return the absolute value of the result.
$scope.computeMismatchDiff=function(summaryItem,type,abs){var defaultFilter=$filter("default"),mismatchType=summaryItem.mismatchTypes[type];if(!mismatchType)return 0;var diff=defaultFilter(mismatchType.NEW,0)+defaultFilter(mismatchType.REGRESSION,0)-defaultFilter(mismatchType.RESOLVED,0);return abs?Math.abs(diff):diff},
// Return a css class based on whether the mismatch count is positive or negative
$scope.mismatchDiffClass=function(summaryItem,type){var val=$scope.computeMismatchDiff(summaryItem,type,!1);return val>0?"postfix-icon icon-arrow-up4 new-error":val<0?"postfix-icon icon-arrow-down5 closed-error":"postfix-icon icon-minus3 existing-error"},$scope.noErrorFilter=function(row){return $scope.showErrorlessReports||row.openMismatches>0},$scope.filterSummaries=function(){$scope.filteredReportSummaries=$scope.reportSummaries.filter($scope.noErrorFilter)},$scope.$watch("showErrorlessReports",$scope.filterSummaries),$scope.init()}]),/** --- Report Detail Controller --- */
angular.module("open.spotcheck").controller("SpotcheckReportCtrl",["$scope","$element","$filter","$location","$timeout","$routeParams","$rootScope","SpotcheckDetailAPI","SpotcheckDefaultFilter",function($scope,$element,$filter,$location,$timeout,$routeParams,$rootScope,SpotcheckDetailAPI,defaultFilter){function filterMismatches(){var filtered=$filter("filter")($scope.state.mismatches,function(mismatch){return $scope.state.filter.passes(mismatch)}),ordered=$filter("sortMismatches")(filtered,$scope.state.filter.orderBy,$scope.state.filter.sortOrder);$scope.state.filteredMismatches=$filter("limitTo")(ordered,$scope.state.filter.limit,$scope.state.filter.offset-1),console.log("limited",$scope.state.filter.limit,$scope.state.filter.offset,$scope.state.filteredMismatches)}$scope.state={loadingReport:!1,mismatches:[],filteredMismatches:[],filter:angular.copy(defaultFilter),summary:null,report:null,dataDetails:null},
// Initialization function
$scope.init=function(){$routeParams.hasOwnProperty("runTime")&&$routeParams.hasOwnProperty("type")&&($scope.state.reportType=$routeParams.type,$scope.state.reportDateTime=$routeParams.runTime,$scope.getReportDetails())},
// Fetch the report via the report detail API
$scope.getReportDetails=function(){SpotcheckDetailAPI.get({reportType:$scope.state.reportType,reportDateTime:$scope.state.reportDateTime},function(response){$scope.state.report=response.details,$scope.state.referenceDateTime=$scope.state.report.referenceDateTime,$scope.state.reportType=$scope.state.report.referenceType,$scope.extractTableData(),$scope.state.summary={mismatchStatuses:$scope.state.report.mismatchStatuses,mismatchCounts:$scope.state.report.mismatchCounts},$timeout(filterMismatches),$scope.state.loadingReport=!1,console.log("report detail received:",$scope.state.report.referenceType,$scope.state.report.referenceDateTime)}),$scope.state.loadingReport=!0,console.log("new report detail requested: ",$scope.state.reportType,$scope.state.reportDateTime)},
// Extracts an array of table rows from the report data
$scope.extractTableData=function(){$scope.state.mismatches=$scope.extractMismatchRows($scope.state.report.observations,$scope.state.reportType),angular.forEach($scope.tableData,function(row){$scope.state.dataDetails[row.mismatchId]=row})},$scope.getMismatchDetails=function(mismatchId){return $scope.state.dataDetails[mismatchId]},$scope.openDetailWindow=function(mismatchRow){$scope.showMismatchDetails(mismatchRow,$scope.getMismatchDetails)},$rootScope.$on("mismatchFilterChange",filterMismatches)}]),/** --- Open Mismatches Controller --- */
angular.module("open.spotcheck").controller("SpotcheckMismatchCtrl",["$scope","$filter","$routeParams","$rootScope","SpotcheckOpenMismatchAPI","SpotcheckOpenMismatchSummaryAPI","SpotcheckDefaultFilter",function($scope,$filter,$routeParams,$rootScope,OpenMMAPI,OpenMMSummaryAPI,defaultFilter){function getOpenMismatchParams(){var params=angular.merge({},$scope.filter);return params.reportType=$scope.reportType,params.mismatchType=[],angular.forEach(params.types,function(requested,type){requested&&params.mismatchType.push(type)}),delete params.statuses,delete params.types,delete params.passes,delete params.filterFunction,params}$scope.unselectedReportType="unselected",$scope.mismatchRows=[],$scope.summary=null,$scope.filter=angular.copy(defaultFilter),$scope.requestCount=0,$scope.lastReceived=0,$scope.init=function(){$scope.reportType=$routeParams.reportType||$scope.unselectedReportType,$scope.getOpenMismatches()},$rootScope.$on("mismatchFilterChange",function(){console.log("mismatch filter change detected",$scope.filter),$scope.getOpenMismatches()}),$scope.getOpenMismatches=function(){if($scope.parameterError=$scope.requestError=!1,"unselected"!==$scope.reportType){var params=getOpenMismatchParams(),reqId=++$scope.requestCount;console.log("getting open mismatches",params),OpenMMAPI.get(params,function(response){
// Do not extract data if the received request was made before the latest received
if(reqId>$scope.lastReceived){var summaries=response.summary.summaryMap;$scope.reportType in summaries&&($scope.summary=summaries[$scope.reportType]),$scope.mismatchRows=$scope.extractMismatchRows(response.observations,response.query.refTypes[0]),$scope.mismatchRows=$filter("sortMismatches")($scope.mismatchRows,$scope.filter.orderBy,$scope.filter.sortOrder),$scope.lastReceived=reqId,console.log("got it")}},function(response){reqId>$scope.lastReceived&&(console.log("uh oh:",response),$scope.tableData=[],$scope.lastReceived=reqId,400===response.status&&1===response.data.errorCode?($scope.parameterError=!0,$scope.parameterErrorVal=response.data.errorData.parameterConstraint.name,$scope.invalidParamDialog(response)):$scope.requestError=!0)})}}}]),angular.module("open.spotcheck").controller("SpotcheckOpenSummaryCtrl",["$scope","$filter","SpotcheckOpenMismatchSummaryAPI",openSummaryCtrl]),angular.module("open.spotcheck").controller("SpotcheckReportCtrl",["$scope","$route","$location","$routeParams","$mdDialog","$mdDateLocale","PaginationModel","SpotcheckMismatchApi","SpotcheckMismatchSummaryApi","SpotcheckMismatchIgnoreAPI","SpotcheckMismatchTrackingAPI","SpotcheckMismatchDeleteAllAPI","SpotcheckMismatchDeleteAllAPI",ReportCtrl]),angular.module("open.spotcheck").factory("SpotcheckMismatchSummaryApi",["$resource",mismatchSummaryApi]),angular.module("open.spotcheck").factory("SpotcheckMismatchApi",["$resource",spotcheckMismatchApi]);var adminModule=angular.module("open.admin",["open.core"]);angular.module("open.admin").controller("DashboardCtrl",["$scope","$routeParams",function($scope,$routeParams){$scope.activeIndex=parseInt($routeParams.view,10)||0,$scope.init=function(){$scope.setHeaderVisible(!0),$scope.setHeaderText("Manage configuration")},$scope.$watch("activeIndex",function(n,o){n!==o&&$routeParams.view!==n&&$scope.setSearchParam("view",$scope.activeIndex)}),$scope.viewMap={environment:0,cache:1,index:2},$scope.init()}]);var adminModule=angular.module("open.admin");adminModule.factory("LogSearchAPI",["$resource",function($resource){return $resource(adminApiPath+"/apiLogs/")}]),adminModule.factory("DataProcessRunsAPI",["$resource",function($resource){return $resource(adminApiPath+"/process/runs/:from/:to")}]),adminModule.factory("DataProcessRunsDetailsAPI",["$resource",function($resource){return $resource(adminApiPath+"/process/runs/id/:id")}]),adminModule.controller("LogsCtrl",["$scope","$routeParams","$timeout","PaginationModel","LogSearchAPI","DataProcessRunsAPI","DataProcessRunsDetailsAPI",function($scope,$routeParams,$timeout,PaginationModel,LogSearchAPI,DataProcessRunsAPI,DataProcessRunsDetailsAPI){$scope.view=parseInt($routeParams.view,10)||0,/** Watch for changes to the current view. */
$scope.$watch("view",function(n,o){n!==o&&$routeParams.view!==n&&$scope.setSearchParam("view",$scope.view)}),
// Api log monitor
$scope.stompClient=null,$scope.now=moment(),$scope.newApiRequestsCount=0,$scope.newApiRequests=[],$scope.connectToSocket=function(){var socket=new SockJS(window.ctxPath+"/sock");$scope.stompClient=Stomp.over(socket),$scope.stompClient.connect({},function(frame){console.log("Connected: "+frame),$scope.now=moment(),$scope.stompClient.subscribe("/event/apiLogs",function(logEvent){$scope.$apply(function(){$scope.newApiRequests.unshift(JSON.parse(logEvent.body)),$scope.newApiRequests.length>2e3&&($scope.newApiRequests=[]),$scope.newApiRequestsCount++})})})},$scope.padLeft=function(n){return n<10?"0"+n:n},$scope.resetRunningLog=function(){$scope.now=moment(),$scope.newApiRequests=[],$scope.newApiRequestsCount=0},$scope.disconnect=function(){null!=$scope.stompClient&&$scope.stompClient.disconnect()},
// Api log search
