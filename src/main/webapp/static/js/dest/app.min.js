...
// for resolve
return mismatchStatuses.indexOf("RESOLVED")!=-1&&(params.fromDate=fromDate),mismatchApi.get(params).$promise.then(parseMismatches)}function parseMismatches(response){var result={pagination:{total:0},mismatches:[]};return result.pagination.total=response.total,angular.forEach(response.result.items,function(mismatch){"NOT_IGNORED"===mismatch.ignoreStatus&&result.mismatches.push(createMismatch(mismatch))}),result}function createMismatch(mismatch){return{id:parseMismatchId(mismatch),status:parseStatus(mismatch),mismatchType:parseMismatchType(mismatch),observedDate:parseObservedDate(mismatch),referenceDate:parseReferenceDate(mismatch),reportDate:parseReportDate(mismatch),issue:parseIssues(mismatch),refType:parseRefType(mismatch),bill:parseBill(mismatch),source:parseRefType(mismatch),calNo:parseCalNo(mismatch),session:parseSession(mismatch),basePrintNo:parseBasePrintNo(mismatch),referenceData:parseReferenceData(mismatch),observedData:parseObserveredData(mismatch),diffLoading:!1,
// TODO: Add CalType to API response.
agendaNo:parseAgendaNo(mismatch),committee:parseCommittee(mismatch)}}function parseSession(mismatch){return mismatch.key.session}function parseBasePrintNo(mismatch){return mismatch.key.basePrintNo}function parseMismatchId(mismatch){return mismatch.mismatchId}function parseReferenceData(mismatch){return mismatch.referenceData}function parseStatus(mismatch){return mismatch.status}function parseObserveredData(mismatch){return mismatch.observedData}function parseMismatchType(mismatch){return mismatchMap[mismatch.mismatchType]}function parseReferenceDate(mismatch){return moment(mismatch.referenceDateTime).format(DATE_FORMAT)}function parseObservedDate(mismatch){return moment(mismatch.observedDateTime).format(DATE_FORMAT)}function parseReportDate(mismatch){return moment(mismatch.reportDateTime).format(DATE_FORMAT)}function parseIssues(mismatch){return mismatch.issueIds.items.join(", ")}function parseRefType(mismatch){return mismatch.referenceType}function parseBill(mismatch){return mismatch.key.printNo||""}function parseCalNo(mismatch){return mismatch.key.calNo||""}function parseAgendaNo(mismatch){return null==mismatch.key.agendaId?"":mismatch.key.agendaId.number+mismatch.key.addendum}function parseCommittee(mismatch){return null==mismatch.key.committeeId?"":mismatch.key.committeeId.name}const DATE_FORMAT="MM-DD-YY h:mm a";var mismatchApi=$resource(adminApiPath+"/spotcheck/mismatches");return{getMismatches:getMismatches}}var openApp=angular.module("open",
// External modules
