...
$scope.getSummaryCountForContentType=function(contentType){return null==$scope.summaryResponse.summary[contentType]?"":$scope.summaryResponse.summary[contentType][$scope.status]||""},$scope.init=function(){resetPagination(),initializeDate(),$scope.datasource.selected=$scope.datasource.values[0],$scope.updateMismatchSummary(),$scope.updateMismatches()},$scope.init()}function mismatchSummaryApi($resource){/**
     ...
     */
function get(datasource,date){return mismatchSummaryApi.get({datasource:datasource,summaryDateTime:date}).$promise.then(createSummary)}function createSummary(response){var summary={OPEN:0,NEW:0,RESOVLED:0,BILL:{OPEN:0,NEW:0,RESOVLED:0},CALENDAR:{OPEN:0,NEW:0,RESOVLED:0},AGENDA:{OPEN:0,NEW:0,RESOVLED:0}};console.log(response);try{summary.OPEN+=response.result.summary.items.REGRESSION.total||0}catch(e){}try{summary.OPEN+=response.result.summary.items.EXISTING.total||0}catch(e){}try{summary.OPEN+=response.result.summary.items.NEW.total||0}catch(e){}try{summary.NEW+=response.result.summary.items.NEW.total||0}catch(e){}try{summary.RESOVLED+=response.result.summary.items.RESOVLED.total||0}catch(e){}try{summary.BILL.OPEN+=response.result.summary.items.REGRESSION.contentTypeCounts.items.BILL||0}catch(e){}try{summary.BILL.OPEN+=response.result.summary.items.EXISTING.contentTypeCounts.items.BILL||0}catch(e){}try{summary.BILL.OPEN+=response.result.summary.items.NEW.contentTypeCounts.items.BILL||0}catch(e){}try{summary.BILL.NEW+=response.result.summary.items.NEW.contentTypeCounts.items.BILL||0}catch(e){}try{summary.BILL.RESOVLED+=response.result.summary.items.RESOVLED.contentTypeCounts.items.BILL||0}catch(e){}try{summary.CALENDAR.OPEN+=response.result.summary.items.REGRESSION.contentTypeCounts.items.CALENDAR||0}catch(e){}try{summary.CALENDAR.OPEN+=response.result.summary.items.EXISTING.contentTypeCounts.items.CALENDAR||0}catch(e){}try{summary.CALENDAR.OPEN+=response.result.summary.items.NEW.contentTypeCounts.items.CALENDAR||0}catch(e){}try{summary.CALENDAR.NEW+=response.result.summary.items.NEW.contentTypeCounts.items.CALENDAR||0}catch(e){}try{summary.CALENDAR.RESOVLED+=response.result.summary.items.RESOVLED.contentTypeCounts.items.CALENDAR||0}catch(e){}try{summary.AGENDA.OPEN+=response.result.summary.items.REGRESSION.contentTypeCounts.items.AGENDA||0}catch(e){}try{summary.AGENDA.OPEN+=response.result.summary.items.EXISTING.contentTypeCounts.items.AGENDA||0}catch(e){}try{summary.AGENDA.OPEN+=response.result.summary.items.NEW.contentTypeCounts.items.AGENDA||0}catch(e){}try{summary.AGENDA.NEW+=response.result.summary.items.NEW.contentTypeCounts.items.AGENDA||0}catch(e){}try{summary.AGENDA.RESOVLED+=response.result.summary.items.AGENDA.contentTypeCounts.items.AGENDA||0}catch(e){}return summary}var mismatchSummaryApi=$resource(adminApiPath+"/spotcheck/mismatches/summary");return{get:get}}function spotcheckMismatchApi($resource){function getMismatches(datasource,contentType,mismatchStatuses,fromDate,toDate,limit,offset){var params={datasource:datasource,contentType:contentType,mismatchStatuses:mismatchStatuses,limit:limit,offset:offset,toDate:toDate};
// for resolve
return mismatchStatuses.indexOf("RESOLVED")!=-1&&(params.fromDate=fromDate),mismatchApi.get(params).$promise.then(parseMismatches)}
// TODO: Sort mismatches?
function parseMismatches(response){var result={pagination:{total:0},mismatches:[]};return result.pagination.total=response.total,angular.forEach(response.result.items,function(mismatch){"NOT_IGNORED"===mismatch.ignoreStatus&&result.mismatches.push(createMismatch(mismatch))}),result}function createMismatch(mismatch){return{id:parseMismatchId(mismatch),status:parseStatus(mismatch),mismatchType:parseMismatchType(mismatch),observedDate:parseDate(mismatch),issue:parseIssues(mismatch),refType:parseRefType(mismatch),bill:parseBill(mismatch),source:parseRefType(mismatch),calNo:parseCalNo(mismatch),
// TODO: Add CalType to API response.
agendaNo:parseAgendaNo(mismatch),committee:parseCommittee(mismatch)}}function parseMismatchId(mismatch){return mismatch.mismatchId}function parseStatus(mismatch){return mismatch.status}function parseMismatchType(mismatch){return mismatchMap[mismatch.mismatchType]}function parseDate(mismatch){return moment(mismatch.observedDateTime).format(DATE_FORMAT)}function parseIssues(mismatch){return mismatch.issueIds.items.join(", ")}function parseRefType(mismatch){return mismatch.referenceType}function parseBill(mismatch){return mismatch.key.printNo||""}function parseCalNo(mismatch){return mismatch.key.calNo||""}function parseAgendaNo(mismatch){return null==mismatch.key.agendaId?"":mismatch.key.agendaId.number}function parseCommittee(mismatch){return null==mismatch.key.committeeId?"":mismatch.key.committeeId.name}const DATE_FORMAT="YYYY-MM-DD h:mm:ss a";var mismatchApi=$resource(adminApiPath+"/spotcheck/mismatches");return{getMismatches:getMismatches}}var openApp=angular.module("open",
// External modules
