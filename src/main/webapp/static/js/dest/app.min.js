...
// }
// else {
return mismatchMap[mismatch.mismatchType]}function parseReferenceDate(mismatch){return moment(mismatch.referenceDateTime).format(DATE_FORMAT)}function parseObservedDate(mismatch){return moment(mismatch.observedDateTime).format(DATE_FORMAT)}function parseReportDate(mismatch){return moment(mismatch.reportDateTime).format(DATE_FORMAT)}function parseIssues(mismatch){return mismatch.issueIds.items.join(", ")}function parseRefTypeLabel(mismatch){return referenceTypeDisplayMap[mismatch.referenceType]}function parseRefType(mismatch){return mismatch.referenceType}function parseBill(mismatch){return mismatch.key.printNo||""}function parseCalNo(mismatch){return mismatch.key.calNo||""}function parseAgendaNo(mismatch){return void 0==mismatch.key.agendaId||null==mismatch.key.agendaId?"":mismatch.key.agendaId.number==-1?"N/A":"DEFAULT"==mismatch.key.addendum?mismatch.key.agendaId.number:mismatch.key.agendaId.number+patternWords(mismatch.key.addendum)}function parseCommittee(mismatch){return null==mismatch.key.committeeId?"":mismatch.key.committeeId.name}function patternWords(input){var lower=input.toLowerCase();return lower.charAt(0).toUpperCase()+lower.slice(1)}const DATE_FORMAT="M-D-YY h:mm a";var mismatchApi=$resource(adminApiPath+"/spotcheck/mismatches");return{getMismatches:getMismatches}}function mismatchSummaryApi($resource){/**
