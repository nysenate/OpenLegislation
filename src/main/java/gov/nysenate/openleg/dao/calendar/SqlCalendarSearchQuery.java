...
public enum SqlCalendarSearchQuery implements BasicSqlQuery {
    ...
    ACTIVE_LIST_FROM(String.format(CALTYPE_SUBQUERY_FROM.getSql(), SqlTable.CALENDAR_ACTIVE_LIST.toString(), ACTIVE_LIST_PREFIX.getSql())),
    SUPPLEMENTAL_FROM(String.format(CALTYPE_SUBQUERY_FROM.getSql(), SqlTable.CALENDAR_SUPPLEMENTAL.toString(), SUPPLEMENTAL_PREFIX.getSql())),

    CALTYPE_SUBQUERY_JOIN(
            "\n\t" + "JOIN %1$s_%2$s_query ON %1$s_%2$s_query.calendar_%3$s_id = %1$s.id"
    ),
    ACTIVE_LIST_PRINT_NO_JOIN(String.format(CALTYPE_SUBQUERY_JOIN.getSql(), ACTIVE_LIST_PREFIX.getSql(), "bills", "active_list")),
    ACTIVE_LIST_BILL_CALENDAR_NO_JOIN(String.format(CALTYPE_SUBQUERY_JOIN.getSql(), ACTIVE_LIST_PREFIX.getSql(), "bcns", "active_list")),
    SUPPLEMENTAL_PRINT_NO_JOIN(String.format(CALTYPE_SUBQUERY_JOIN.getSql(), SUPPLEMENTAL_PREFIX.getSql(), "bills", "sup")),
    SUPPLEMENTAL_BILL_CALENDAR_NO_JOIN(String.format(CALTYPE_SUBQUERY_JOIN.getSql(), SUPPLEMENTAL_PREFIX.getSql(), "bcns", "sup")),
    SUPPLEMENTAL_BILL_SECTION_CODE_JOIN(String.format(CALTYPE_SUBQUERY_JOIN.getSql(), SUPPLEMENTAL_PREFIX.getSql(), "scode", "sup")),
