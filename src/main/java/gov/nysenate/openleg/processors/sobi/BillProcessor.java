...
public class BillProcessor extends SOBIProcessor
{
    ...
    protected static Pattern sameAsPattern = Pattern.compile("Same as( Uni\\.)? ([A-Z] ?[0-9]{1,5}-?[A-Z]?)(, ([A-Z] ?[0-9]{1,5}-?[A-Z]?))?");
    ...
    private IngestCache<Bill> ingestCache;
    ...
    public void process(SOBIFragment sobiFragment) {
        IngestCache<Bill> billCache = new IngestCache<>();
        ...
        for (SOBIBlock block : blocks) {
            ...
            try {
                switch (block.getType()) {
                    ...
                    case PROGRAM_INFO: applyProgramInfo(data, baseBill, specifiedVersion, date); break;
    ...
    public void applyBillInfo(String data, Bill baseBill, BillAmendment specifiedAmendent, Date date) throws ParseError {
        ...
        else if (!specifiedAmendent.isPublished()) {
            // FIXME: Related to http://dev.nysenate.gov/issues/7844
            specifiedAmendent.setPublishDate(date);
            baseBill.setActiveVersion(specifiedAmendent.getVersion());
        }
    ...
    public void applyBillEvent(String data, Bill baseBill, BillAmendment specifiedAmendment, Date date) throws ParseError {
        ...
        Set<DateTime> eventDateTimeSet = new HashSet<>();
        ...
        for (String line : data.split("\n")) {
            ...
            if (billEvent.find()) {
                try {
                    ...
                    // Increment the time by 1 second in the event of any date clashes. This is
                    // primarily to enforce a strict chronological ordering.
                    DateTime eventDateTime = new DateTime(eventDate);
                    if (eventDateTimeSet.contains(eventDateTime)) {
                        eventDateTime = eventDateTime.plusSeconds(1);
                    }
                    BillAction action = new BillAction(eventDateTime.toDate(), eventText, baseBill, specifiedAmendment.getVersion());
                    actions.add(action);
                    eventDateTimeSet.add(eventDateTime);
 ...
 */
    public void applySameAs(String data, BillAmendment specifiedAmendment, Date date) {
        if (data.trim().equalsIgnoreCase("No same as") || data.trim().equalsIgnoreCase("DELETE")) {
            ...
            if (sameAsMatcher.find()) {
                ...
                List<String> sameAsMatches = new ArrayList<>();
                sameAsMatches.add(sameAsMatcher.group(2));
                sameAsMatches.add(sameAsMatcher.group(4));
    ...
    public void applyCosponsors(String data, BillAmendment activeAmendment, Date date) {
        ...
        // The cosponsor info is always to a base bill version.
        // We can use the currently active version instead.
        activeAmendment.setCoSponsors(coSponsors);
        activeAmendment.setModifiedDate(date);
    }

    /**
     * Applies data to bill multi-sponsors. Expects a comma separated list andfully replaces
     ...
     * @throws ParseError
     */
    public void applyProgramInfo(String data, Bill bill, String version, Date date) {
        ...
        for(String line : data.split("\n")) {
            ...
            if (voteHeader.find()) {
                ...
                try {
                    ...
                    vote = new Vote(specifiedAmendment, voteDateFormat.parse(voteHeader.group(2)), Vote.VOTE_TYPE_FLOOR, "1");
                    vote.setPublishDate(date);
                    for (Vote oldVote : specifiedAmendment.getVotes()) {
