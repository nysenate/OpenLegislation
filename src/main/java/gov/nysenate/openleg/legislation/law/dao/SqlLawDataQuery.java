...
public enum SqlLawDataQuery implements BasicSqlQuery
{
    ...
    SELECT_LAW_DOCUMENT(
        "WITH max_date AS (\n" +
        "    SELECT max(published_date) AS pub_date FROM " + SqlTable.LAW_DOCUMENT + "\n" +
        "    WHERE document_id = :docId AND published_date <= :endPublishedDate" +
        ")\n" +
        "SELECT * FROM max_date, " + SqlTable.LAW_DOCUMENT + "\n" +
    ...
    SELECT_LAW_TREE(
        "WITH max_date AS (\n" +
        "    SELECT max(published_date) AS pub_date FROM " + SqlTable.LAW_TREE + "\n" +
        "    WHERE law_id = :lawId AND published_date <= :endPublishedDate" +
        ")\n" +
        ...
        "       t.parent_doc_id\n" +
        "FROM max_date, " + SqlTable.LAW_TREE + " t\n" +
        "LEFT JOIN " + SqlTable.LAW_DOCUMENT + " d1 \n" +
    ...
    UPDATE_LAW_INFO(
        "UPDATE " + SqlTable.LAW_INFO + "\n" +
        "SET chapter_id = :chapterId, law_type = :lawType, name = :name\n" +
        "WHERE law_id = :lawId"
    ),
    INSERT_LAW_INFO(
        "INSERT INTO " + SqlTable.LAW_INFO + " (law_id, chapter_id, law_Type, name)\n" +
