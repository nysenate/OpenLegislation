...
enum ApiUserQuery implements BasicSqlQuery {
    ...
    SELECT_API_USERS(
            "SELECT u.apikey, u.authenticated, u.num_requests, u.email_addr, u.org_name, u.users_name, u.reg_token, " +
                    "array(SELECT r.role FROM " + SqlTable.API_USER_ROLE + " r WHERE r.apikey = u.apikey) AS roles, " +
                    "array(SELECT s.subscription_type FROM " + SqlTable.API_USER_SUBSCRIPTION +
                    " s WHERE s.apikey = u.apikey) AS subscriptions\n" +
                    "FROM " + SqlTable.API_USER + " u\n"
    ),
    SELECT_BY_EMAIL(
    ...
    INSERT_API_USER_ROLE(
            "INSERT INTO " + SqlTable.API_USER_ROLE + "\n" +
                    "       (apikey,  role)\n" +
                    "VALUES(:apiKey, :role)"
    ),
    DELETE_API_USER_ROLE(
            "DELETE FROM " + SqlTable.API_USER_ROLE + "\n" +
                    "WHERE apikey = :apiKey;"
    ),
    INSERT_API_USER_SUBSCRIPTION(
            "INSERT INTO " + SqlTable.API_USER_SUBSCRIPTION + "\n" +
                    "      (apikey, subscription_type)\n" +
                    "VALUES(:apiKey, :subscription_type::" + SqlSchema.USER + "apiuser_subscription_type)"
    ),
    DELETE_API_USER_SUBSCRIPTION(
            "DELETE FROM " + SqlTable.API_USER_SUBSCRIPTION + "\n" +
                    "WHERE apikey = :apiKey AND subscription_type = :subscription_type::" + SqlSchema.USER + "apiuser_subscription_type"
    ),
    ...
    SELECT_API_USERS_BY_SUBSCRIPTION(
            "SELECT a.apikey, a.authenticated, a.num_requests, a.email_addr, a.org_name, a.users_name, a.reg_token, " +
                    "array(SELECT r.role FROM " + SqlTable.API_USER_ROLE + " r WHERE r.apikey = a.apikey) AS roles," +
                    "array(SELECT s.subscription_type FROM " + SqlTable.API_USER_SUBSCRIPTION +
                    " s WHERE s.apikey = a.apikey) AS subscriptions\n" +
                    "FROM (SELECT *" + "\n" + "FROM " + SqlTable.API_USER + " u NATURAL JOIN " +
                    SqlTable.API_USER_SUBSCRIPTION + " s" + "\n" +
                    "WHERE u.apikey = s.apikey ) as a" + "\n" +
                    "WHERE a.subscription_type = :subscription_type::" + SqlSchema.USER + "apiuser_subscription_type"
    );
